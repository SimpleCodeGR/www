<?php
    if ( isset($_GET['app']) ) {
        ...
        if ($result = $mysqli->query("(SELECT * FROM `hm_insta` WHERE `typos` = 'base64' ORDER BY id ASC LIMIT 9) ORDER BY seira ASC")) {
            $row_count = 0;
            echo '{"images":[';
            ...
            $.getJSON( "https://www.instagram.com/"+insta_username+"/?__a=1", function( data ) {
                ...
                for (i=0; i < insta_temp1.length; i++) {
                    ...
                    toDataURL(insta_fotos[i], i, function(dataUrl, seira_temp) {
                        ...
                        jQuery.ajax({  
                            type: "POST",
                            url: 'https://www.highmonkeys.gr/lab/insta_controller.php',
